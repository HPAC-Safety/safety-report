...
| `POST /api/v1/uploads` | Mint an upload as a file is attached; JSON with the declared content type and exact size, never a filename | `201` with an opaque upload ID, kind, and a pre-signed PUT to `quarantine/<upload id>` signed for that type and size, living at most 15 minutes; or `400` with a safe rejection reason. Requires a member bearer token, rate limited, stores nothing identifying the member, and writes nothing. The browser then PUTs the file straight to storage ([ADR-0126](decisions/ADR-0126-an-attachment-uploads-straight-to-quarantine-by-pre-signed-put.md)). |
