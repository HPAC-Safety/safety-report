...
| Worker host | Claims typed outbox messages safely (`OutboxClaimer`, `FOR UPDATE SKIP LOCKED`, #271) and translates every answer with a value — narrative included — into its second official language via the same `ITranslator` port question authoring uses (ADR-0080). No summary or per-attachment handler exists yet. | Implement the summary handler and per-attachment handlers. |
