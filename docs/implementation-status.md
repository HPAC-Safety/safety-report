...
There are 14 tables: `reports`, `report_answers`, `report_files`,
`summaries`, `questions`, `question_revisions`, `question_choices`,
`report_comments`, `report_comment_revisions`, `report_private_notes`,
`report_private_note_revisions`, `pending_import_logic`, `audit_log`, and
`outbox_messages`. Six read-only views carry the read rules
...
private note revision's writer) holds
