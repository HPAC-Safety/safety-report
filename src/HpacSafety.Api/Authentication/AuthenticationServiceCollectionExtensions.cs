...
public static class AuthenticationServiceCollectionExtensions
{
        ...
        services.AddAuthorizationBuilder()
            ...
            .AddPolicy(HpacPolicies.Reviewer, policy => policy
                ...
                .RequireAssertion(context =>
                    MemberRoles.EffectiveRole(context.User, options.RoleClaimType) >= Core.Features.Moderation.MemberRole.SafetyOfficer))
            .AddPolicy(HpacPolicies.Administrator, policy => policy
                .RequireAuthenticatedUser()
                .RequireAssertion(context =>
                    MemberRoles.EffectiveRole(context.User, options.RoleClaimType) >= Core.Features.Moderation.MemberRole.Administrator));
    ...
    private static TokenValidationParameters DevelopmentParameters(HpacAuthenticationOptions options)
    {
        ...
        if (System.Text.Encoding.UTF8.GetByteCount(options.DevelopmentSigningKey) < DevelopmentTokenIssuer.MinimumKeyBytes)
        {
            throw new InvalidOperationException(
    ...
    private static TokenValidationParameters ProviderParameters(HpacAuthenticationOptions options)
    {
        ...
        return Common(options, options.Issuer ?? options.Authority, signingKey: null);
    }
    ...
    /// </summary>
    private static JwtBearerEvents ProblemDetailsEvents() => new()
    {
