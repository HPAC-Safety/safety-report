...
| `report_answers` | ID, report ID, question ID, exact question revision ID, privacy snapshot, nullable choice ID (single-select, multi-select, type-ahead; ADR-0128), nullable string value, the locale it was given in, nullable second-language value and its source (`auto`, `human`, `choice`, or `fixed`), the translation mode fixed at submission (`none`, `choice`, `machine`, or `fixed`, ADR-0112, ADR-0127), answered/recorded timestamp, Deleted. **Every answer that is not a choice is one string**; a choice answer names its choice and stores no wording, reading both labels from it (ADR-0128). A boolean holds the reporter's word, `yes`/`no` or `oui`/`non` (ADR-0127), a date/time holds ISO 8601 (ADR-0072). Includes skipped shown questions, including file-upload controls. |
