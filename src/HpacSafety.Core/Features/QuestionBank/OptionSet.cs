...
#pragma warning restore CS8618
    ...
    public IReadOnlyList<QuestionOptionInput> AsRevisionOptions() =>
        [.. Items.Select(item => new QuestionOptionInput(item.Code, item.LabelEn, item.LabelFr, item.Id))];

    private int NextDisplayOrder() =>
        _items.Count == 0 ? 0 : _items.Max(item => item.DisplayOrder) + 1;
