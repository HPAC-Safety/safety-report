...
- English is the source of truth; French is generated in CI and reviewed by a
  human. Never hand-edit `locales/fr-CA.json` or `locales/fr-CA.meta.json` —
  both are regenerated by `tools/translate-locale.mjs`, and an edit to either
  is overwritten by the next run. Fix the English, or pin the term in
...
- **CI translates on `main` only, and never on a pull request.** The `i18n` job
  runs `translate-locale.mjs --check`, which reads the locale files and
...
| How does the French get generated? | `docs/localization.md`, and `tools/translate-locale.mjs` |
...
required. Three of them — `coverage`, `web`, `e2e` — currently no-op with a
notice because the thing they would verify has not been written yet; each is
filled in by its own issue. `i18n` verifies locale translation parity for real
(#10); its two lint steps still skip until #8 lands the locale files. The deploy
workflows are wired but fail at the AWS step, because the AWS environment does
not exist yet.
