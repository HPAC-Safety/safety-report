...
public sealed partial class DeepLTranslator : ITranslator
{
        ...
        IReadOnlyList<string> texts, Locale source, Locale target, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            client.DefaultRequestHeaders.Authorization =
                new System.Net.Http.Headers.AuthenticationHeaderValue("DeepL-Auth-Key", _options.ApiKey);
    ...
    private static string SourceCodeFor(Locale locale) =>
        SourceCodes.TryGetValue(locale.Code, out var code)
            ? code
    ...
    private static string TargetCodeFor(Locale locale) =>
        TargetCodes.TryGetValue(locale.Code, out var code)
            ? code
            : throw new TranslationUnavailableException($"No translation language is configured for '{locale.Code}'.");
    ...
    private sealed record DeepLRequest(
        ...
        [property: JsonPropertyName("formality")] string Formality,
        [property: JsonPropertyName("preserve_formatting")] bool PreserveFormatting,
        [property: JsonPropertyName("tag_handling")] string TagHandling,
        [property: JsonPropertyName("ignore_tags")] IReadOnlyList<string> IgnoreTags);

    private sealed record DeepLResponse(
        [property: JsonPropertyName("translations")] IReadOnlyList<DeepLTranslation> Translations);
