...
| Submission | Implemented end to end: `POST /api/v1/reports` (#14) validates in order, reloads known superseded revisions, sniffs and validates each named upload from its stored size and the bytes sniffing needs, then claims it by a server-side copy into the report's original compartment (refusing expired and invalid ones by id before writing anything, ADR-0098, ADR-0126), enforces required/select/file-shape rules, and atomically persists the report, every answer (immutable `value`/`locale`, ADR-0080), file rows with the reporter's sanitized filename (ADR-0097), and three outbox messages (summary, per-file attachment, answer-translation — the Worker claims and processes the last of these, #271). Each attachment is minted as it is attached through `POST /api/v1/uploads`, which returns a pre-signed PUT signed for the declared type and exact size, and the browser sends it straight to `quarantine/<upload id>`; `DELETE` erases every version (#360, #462, ADR-0096, ADR-0126); the form's `AttachmentField` shows each upload's indicator and Cancel, and holds Next/Submit while any is in flight. A type-ahead value the question does not offer is added to that question's choices in the same transaction (ADR-0063, ADR-0095). | — |
...
| Media images | Signature sniffing, per-kind size limits (250 MB video, 25 MB image or document), private storage, and decode/re-encode metadata stripping are implemented and tested. An upload goes straight to quarantine through a pre-signed PUT and is sniffed and validated when a submission claims it ([ADR-0126](decisions/ADR-0126-an-attachment-uploads-straight-to-quarantine-by-pre-signed-put.md), #462). | — |
...
| Blob access | One adapter, `S3BlobStore`: S3 through the ECS task role in AWS, RustFS in docker-compose for development (ADR-0096, ADR-0110). Reviewer URLs are short-lived, forced-download, signed for the browser-reachable host, and carry the reporter's sanitized filename (ADR-0097); reviewer links are derivative-only for images/video and original-only for documents. `FileSystemBlobStore` is removed; `CreateUploadUrl` mints a reporter's one pre-signed PUT, and a staff private attachment's, only for a key whose compartment takes direct uploads (ADR-0126, ADR-0135). A private attachment lives under `<report id>/private/` and is signed a download only by `PrivateAttachmentLink`. | The live S3 path is untested until AWS credentials exist (#30). |
...
There are 15 tables: `reports`, `report_answers`, `report_files`,
`summaries`, `questions`, `question_revisions`, `question_choices`,
`report_comments`, `report_comment_revisions`, `report_private_notes`,
`report_private_note_revisions`, `report_private_attachments`,
`pending_import_logic`, `audit_log`, and `outbox_messages`. Six read-only views carry the read rules
...
`summaries.approved_by_subject`, a comment's author, a hide's reviewer, a
private note revision's writer, a private attachment's adder and remover) holds
an opaque token subject with no foreign key.

38 migrations build that shape. The landmarks:
