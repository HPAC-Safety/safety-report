...
**Retired by [ADR-0065](ADR-0065-no-user-records-identity-is-the-token-subject.md).**
`admin_users` no longer exists, so nothing below this heading still happens.
The `DevelopmentAdminSeed` class survives only because a committed migration
calls it and past migrations are never edited; it writes to a table that a
later migration drops. It is kept for the historical record.
