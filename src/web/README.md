...
after a merge to main (ADR-0021) and is never hand-authored; until it
exists, `loadCatalogue` falls back to English for every key, so the app
runs correctly with only `en-CA.json` present. The chosen locale persists to
