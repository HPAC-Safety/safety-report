Feature: Web, localization, and design
  ...
  Scenario: The public and admin sites are independently deployed
    ...
    And neither requires a SPA framework, client router, Node production
      server, or bundler
  ...
  Scenario: Switching the language toggle rerenders without losing answers
    ...
    Then labels, help, validation, navigation, and formatting rerender in the
      new locale
  ...
  Scenario: Application chrome strings come from committed locale catalogues
    ...
    Then it comes from a committed locale catalogue with key parity between
      en-CA and fr-CA
    And no user-facing literal appears directly in code

  Scenario: Question content comes from the bilingual database revision
    Given a question revision has English and French labels, help, and
      options authored by an Administrator
  ...
  Scenario: Only publication consent is marked required on the form
    ...
    And every optional question offers a natural blank/skipped state with no
      coerced answer
    And consent_publish has no selected default and requires an explicit yes
      or no

  Scenario: The form preserves local state and warns about attachments
    Given a reporter is filling out the form
    Then the browser preserves locale, revision IDs, and answers locally for
      15 days
    And a privacy explanation is shown before submission
    And attachment selection appears last with type/count/size guidance and a
      warning that files are not restored after reload

  Scenario: Submission feedback is bounded and idempotent-looking
    Given a reporter submits the form
    When the request is in flight
    Then the UI shows bounded progress and disables accidental duplicate
      clicks
  ...
  Scenario: Client validation never replaces server validation
    ...
    Then the API's validation is authoritative regardless of what the client
      allowed
  ...
  Scenario: Admin pages distinguish private, ordinary, and output content
    ...
    Then private context, ordinary report content, summary output, processing
      failures, approval state, safe image/video derivatives, and unredacted
      private document downloads are all visibly distinguished
    And dangerous actions require clear confirmation
    And editing either summary text visibly invalidates approval
    And question editing explains that saving always creates a new immutable
      revision

  Scenario: Assets are self-hosted, never loaded from third-party CDNs
    Given the site renders fonts, styles, or imagery
    Then Aleo, Poppins, and other assets are served as self-hosted WOFF2/
      committed files
    And no asset is loaded from a third-party CDN
    And the current logo is a placeholder that may only be replaced with an
      approved HPAC asset
  ...
  Scenario: Dark mode is a token redefinition, not duplicated markup
    ...
    Then the same component markup is used with redefined CSS custom-property
      tokens
    And contrast, focus, error, disabled, and success states work in both
  ...
  Scenario: The form meets baseline accessibility requirements
    ...
    And motion respects reduced-motion and touch targets/contrast are
      sufficient
