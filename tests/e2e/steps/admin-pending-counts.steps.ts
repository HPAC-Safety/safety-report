...
Given(
	...
	async ({ page }, reports: string, answers: string) => {
...
Then(/^the (manage-[a-z-]+) option shows a count of (\d+)$/, async ({ page }, name: string, count: string) => {
	await expect(option(page, name)).toHaveAccessibleName(`${OPTIONS[name]} ${count} waiting`)
})

Then(/^the (manage-[a-z-]+) option shows no count$/, async ({ page }, name: string) => {
