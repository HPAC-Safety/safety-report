...
description: Apply HPAC safety-report persistence, EF Core, migration, encryption, identifier, time, and seed-data rules. Use when changing entities, tables, mappings, migrations, value converters, TinyId keys, occurrence dates or times, encrypted fields, domain-code storage, or the seeded question bank.
---
...
- Encrypt every Restricted field in the application before PostgreSQL sees it,
  through `IFieldCipher` and a value converter. Never create a clear-text helper
  column.
...
- Reproduce generated `docs/form-spec.md` exactly in the seeded question bank,
  and keep the test that proves it. Regenerate the spec with
