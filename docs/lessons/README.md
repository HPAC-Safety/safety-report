...
| [0010 — A coverage gate found in CI, not before the pull request](0010-a-coverage-gate-found-in-ci-not-before-the-pull-request.md) | A pull request with every test green failed CI's branch-coverage ratchet | Superseded by 0025 |
...
| [0024 — A Translate that only filled the empty side](0024-a-translate-that-only-filled-the-empty-side.md) | Translate stayed disabled on every existing question, because every scenario began with one language empty | `REQ-QB-172`, `REQ-QB-173`, `REQ-QB-164`, `REQ-QB-170` |
| [0025 — A local gate that re-implemented CI disagreed with it](0025-a-local-gate-that-re-implemented-ci-disagreed-with-it.md) | Two local coverage scripts re-implemented CI's ratchet on macOS and often disagreed with it; the other pull request checks ran by hand or not at all | `deliver-change`, `deliver-hpac-change` |
