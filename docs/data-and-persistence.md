...
| `question_choices` | ID, question ID, stable choice code, nullable EN/FR label (at least one), sort order, reporter-added marker, needs-translation marker, the locale a reporter typed it in (null for an administrator's choice), Deleted. Unique question + code, including across a removed choice, so a removed code is never revived. **Mutable and outside the revision chain** — editing choices never revises or forks the question, and a fork copies every row to the replacement. Only a reporter-added choice may lack a label (ADR-0095). |
