...
| Typeform import/export | `TypeformQuestionMapper` is implemented and tested against the organization's real `formENG.json`/`formFR.json` export pair (checked in as fixtures) — English-led field mapping, statement/group/contact_info flattening, the recap-screen skip, unsupported-type rejection, and pending-logic detection for any real branching condition (ADR-0077, amended by ADR-0078). No API endpoint, admin UI, `pending_import_logic` persistence, or export builder exists yet; nothing is wired to the question bank or a database. | Add the import preview endpoint, `pending_import_logic` table, admin UI, seed regeneration, and export. |
