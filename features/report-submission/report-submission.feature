Feature: Report submission
  ...
  Background:
    ...
    And it accepts multipart/form-data with one report JSON part, zero or more
      files parts, and one Turnstile response token
  ...
  Scenario: The browser holds report state locally until submission
    ...
    Then the selected locale, shown question-revision IDs, and entered answers
      exist only in local browser storage with a 15-day expiry
    And image, video, and document attachments are never placed in browser
      storage
    And no server draft, report ID reservation, upload token, or resumable
      upload protocol exists
  ...
  Scenario: One answer entry per shown answer-producing revision
    Given the client says it showed the reporter a set of answer-producing
      revisions
    When the reporter submits the form
    Then the submission DTO contains exactly one answer entry for each of
      those revisions
    And textual/scalar answers use "value", selection answers use
      "option_codes", and file-upload answers use zero-based indexes into the
  ...
  Scenario: A submission may answer a known superseded revision
    ...
    Then the API validates the answer against that revision's historical
      type, options, and privacy
  ...
  Scenario: Reporter-visible errors never echo submitted content
    ...
    And it never echoes an answer, client filename, Turnstile token, or
      credential
  ...
  Scenario: Accepted attachments are streamed into quarantine under a bound
    ...
    And streams at most 50 MB into the quarantine compartment while computing
      the actual byte count and inspecting its signature
  ...
  Scenario: A valid submission is persisted atomically
    ...
    Then one database transaction creates the report and consent projection,
      one answer per shown answer-producing revision including skips,
      report-file metadata for successfully quarantined blobs, one
      summarization outbox item, and one independent attachment-processing
      outbox item per file
  ...
  Scenario: A failed transaction leaves no visible report and no leaked blobs
    ...
    And any already-written quarantine blobs are unreferenced and expire
      through the storage lifecycle rule
  ...
  Scenario: A successful submission returns an opaque accepted receipt
    ...
    Then the response is 202 Accepted with an opaque report ID and the status
      "submitted"
  ...
  Scenario: Submission is rejected without valid abuse-control checks
    ...
    When Turnstile verification fails, or Turnstile is required but
      unavailable or misconfigured, or the per-IP rate limit is exceeded
    Then the API rejects the request
    And a rate-limited request receives 429 with a safe retry signal
    And the client IP used for rate limiting comes only from explicitly
      trusted proxy headers and is never stored on the report
