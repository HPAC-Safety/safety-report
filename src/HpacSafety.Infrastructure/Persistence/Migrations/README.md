...
erDiagram
    ...
    questions {
        ...
        char(11) choices_depend_on_question_id FK "nullable; the question whose answer filters this one's choices (ADR-0146)"
...
| View | What it holds | Last defined by |
|---|---|---|
| `public_reports` | The whole publication rule, and the public DTO's allowlist. Only a live, consented, Published report with a live, approved, nonblank pair. `Report.IsPublishable` mirrors it for tests; the view is the authority. | `20260925164215_ReportIsPendingPublishedOrUnpublished` |
| `public_report_comments` | Each visible comment's current revision, on a report in `public_reports` (ADR-0114). | `20260924170847_AddReportComments` |
| `public_report_media` | Each public image or video derivative, and each validated document on a report whose media consent names documents, on a report in `public_reports` (ADR-0117, ADR-0119). | `20260924205238_OfferPublishedDocuments` |
| `admin_report_queue` | Every live report's state and timing for the admin list, with `is_stuck` and `needs_action`. | `20260925164215_ReportIsPendingPublishedOrUnpublished` |
| `answers_awaiting_translation` | Every live answer marked for machine translation that has no second language yet. Choice answers are left out. | `20260925230357_NameEachAnswersChoice` |
| `admin_pending_counts` | One row of counts for the Admin menu, including type-ahead values awaiting review. | `20260925235946_ReviewTypeAheadValues` |
...
| `20260922014044_AddGroupedUnderQuestionId` | Added nullable `question_revisions.grouped_under_question_id`, a restricted foreign key to `questions` with its index: the group heading a question sits under (ADR-0076). |
| `20260922022256_AddAllowsReporterAdditions` | Added `question_revisions.allows_reporter_additions` (default `false`), so a multi-select could take reporter additions (ADR-0077). `GiveEachQuestionItsOwnChoices` later dropped it (ADR-0095). |
| `20260922033939_AddPendingImportLogic` | Added `pending_import_logic` (import batch, field `ref` and title, the raw Typeform logic JSON, created time), indexed by batch: a note for each branching rule an import could not map (ADR-0077, ADR-0078). |
| `20260922053228_SeedQuestionBankFromTypeformFixtures` | No schema change. Seeds the question bank from `QuestionBankSeed.Questions` through `QuestionBankSeedWriter`, read when the migration runs. |
| `20260922152717_WorkerTranslatedAnswers` | Replaced `report_answers.needs_translation` and its index with `translation_source` and a partial index on `answered_at` over answers that have a value and no second language yet: the Worker's translation queue (ADR-0080). |
| `20260922153954_AddAttachmentAndTranslationOutboxTypes` | Recreated `ck_outbox_messages_type` to allow `process_attachment` and `translate_answers` beside `summarize_report`. |
...
| `20260924050906_ReviewActionsRejectionNote` | Added nullable `reports.rejection_note` (up to 2000 characters), a reviewer's note shown only in the admin view (ADR-0105). The report and summary concurrency tokens map to PostgreSQL's `xmin`, which needs no column. `ReportIsPendingPublishedOrUnpublished` later renamed it `unpublish_note`. |
| `20260924055856_RecordSummaryLanguageSources` | Added `summaries.source_en` and `source_fr` (`generated`, `human`, or `machine`; default `generated`, with a check each), and marked both `human` on every hand-written (`manual`) summary (`Sql/20260924055856_RecordSummaryLanguageSources.sql`, ADR-0108). |
...
| `20260924180636_CreateAdminQueueViews` | No table change. Creates `admin_report_queue` (every live report's state and timing, with `is_stuck` and `needs_action`), `answers_awaiting_translation` (the Worker's translation queue, matching its partial index), and `admin_pending_counts` (one row of admin work counts for the Admin menu) (`Sql/20260924180636_CreateAdminQueueViews.sql`, #418, ADR-0116). |
| `20260924190336_ShowPublishedMedia` | Added `reports.consent_media`, and `report_files.hidden_at` and `hidden_by_subject` (set together, by check), and allowed the `consent_media` question role. Seeds the media-consent system question once, and creates `public_report_media`: each public image or video, only while its report is in `public_reports` (`Sql/20260924190336_ShowPublishedMedia.sql`, ADR-0117). |
| `20260924205238_OfferPublishedDocuments` | Added `reports.consent_documents` and `report_files.validated_at` (documents only, by check). Rewords the media-consent question to name documents, as a new revision. Recreates `public_report_media` to add a validated document on a report whose `consent_documents` is true (`Sql/20260924205238_OfferPublishedDocuments.sql`, ADR-0119). |
| `20260925164215_ReportIsPendingPublishedOrUnpublished` | Renamed `reports.rejection_note` to `unpublish_note`, and replaced `ck_reports_status` with `submitted`, `summarizing`, `summary_failed`, `pending`, `published`, and `unpublished`. Existing rows map as follows: any report past summarizing without publication consent becomes `unpublished`; otherwise `pending_review` becomes `pending`, `rejected` becomes `unpublished`, and `approved` becomes `published`. Replaces `public_reports` to require `status = 'published'`, and `admin_report_queue` so an Unpublished report never needs action (`Sql/20260925164215_ReportIsPendingPublishedOrUnpublished.sql`, ADR-0125). |
