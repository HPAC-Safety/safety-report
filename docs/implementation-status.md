...
| Authentication | Implemented server-side. JWT bearer validation, three role policies, `GET /api/auth/config`, `GET /api/auth/me`, and a Development-only token issuer; the admin endpoints require the Administrator policy. The web session is still a `sessionStorage` marker and does not yet present a token. | Build the token-bearing web session; add the reporter-submission policy with the submission endpoint. |
