...
| Documents | PDF, DOC, DOCX, RTF, Markdown, text, and ODT are sniffed, validated, and ingested as private originals with no derivative (#310). No authorized forced-download endpoint yet (#311). | Wire the reviewer download endpoint; never extract, anonymize, send to LLM, or publish. |
