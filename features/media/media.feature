Feature: Attachments
  ...
  Background:
    Given the maximum attachment count is configurable and defaults to five
      across all attachment kinds
  ...
  Scenario: Declared content type must agree with detected content type
    Given an attachment's declared content type differs from its detected,
      allowlisted type
    When the API validates the attachment
    Then the API rejects the attachment
    And the file extension and client filename are never trusted as the basis
      for acceptance
  ...
  Scenario: The client filename never leaves the HTTP boundary
    ...
    Then the client filename is not persisted, logged, placed in an
      exception, used in a key, sent to the model, or returned to an admin
    And the object key encodes only an opaque report/file identity and a
      managed compartment
  ...
  Scenario: An accepted attachment starts in a private quarantine compartment
    ...
    Then the API mints an opaque filename, streams the file through a bounded
      counter and signature sniffer, and writes accepted bytes to a private
      quarantine key

  Scenario: Unreferenced quarantine blobs expire automatically
    Given a quarantine blob was written during a submission whose transaction
      never committed
  ...
  Scenario: Every image is re-encoded to strip metadata
    ...
    Then the image is decoded and re-encoded into a supported safe
      representation
    And EXIF, GPS, profiles, comments, thumbnails, and other metadata are
      removed
  ...
  Scenario: Every video is remuxed or transcoded to strip metadata
    ...
    When the Worker produces its derivative
    Then the video is decoded/remuxed or transcoded through a controlled
      toolchain that removes container metadata, location, device, creation,
      and filename fields
    And a byte-for-byte copy of the original video is never used as the
      derivative
  ...
  Scenario: A document is validated and scanned but never transformed
    ...
    Then the Worker scans it for known malware and validates its actual
      format, including internal package shape for DOCX/ODT and bounded text
      decoding for Markdown/plain text
    And the Worker never extracts its text, and the document is never sent to
      the model and never published
    And the document remains the reporter-supplied original, available for
      private download, and the review UI labels it as unredacted private
      evidence
  ...
  Scenario: Each attachment fails and processes independently of the report
    ...
    And the slow or corrupt file neither rolls back the valid report nor
      forces an additional AI call
  ...
  Scenario: A reviewer gets a short-lived URL only for successfully processed
    ...
    And the response forces download with a server-minted display name and
  ...
  Scenario: A reviewer downloads a validated document as an unredacted original
    ...
    And the reviewer is warned that the document is unredacted before
      download

  Scenario: A failed attachment is inaccessible to reviewers
    Given signature validation, malware scanning, decoding, metadata removal,
      re-encoding/remuxing, writing, or verification fails for an attachment
